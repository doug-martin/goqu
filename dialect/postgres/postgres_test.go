...
func (pt *postgresTest) TestUpdate() {
	...
	var id uint32
	found, err = ds.Where(goqu.C("int").Eq(11)).Returning("id").Update(goqu.Record{"int": 9}).ScanVal(&id)
	assert.NoError(t, err)
	assert.True(t, found)
...
func (pt *postgresTest) TestInsertIgnore() {
	t := pt.T()
	ds := pt.db.From("entry")
	now := time.Now()
...
func (pt *postgresTest) TestInsertConflict() {
	t := pt.T()
	ds := pt.db.From("entry")
	now := time.Now()
